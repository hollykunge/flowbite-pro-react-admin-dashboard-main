...
// 尝试加载URL
function tryLoadURL() {
  // 从环境变量中获取开发服务器URL，如果没有则使用默认值
  const startUrl = process.env.ELECTRON_START_URL || "http://localhost:5173";
  console.log(`从环境变量获取的开发服务器URL: ${startUrl}`);

  // 设置超时时间，确保即使无法连接到开发服务器，也能显示错误页面
  const timeout = setTimeout(() => {
    console.log("连接开发服务器超时，显示错误页面");
    showErrorPage();
  }, 10000); // 10秒超时
  ...
  function showErrorPage() {
    if (mainWindow) {
      try {
        ...
        if (require("fs").existsSync(errorHtmlPath)) {
          console.log("加载错误页面:", errorHtmlPath);
          mainWindow
            .loadFile(errorHtmlPath)
            .then(() => {
      ...
      mainWindow
        ...
        <head>
          ...
          <style>
            ...
            h1 { color: #e53e3e; }
            button { background: #3182ce; color: white; border: none; padding: 10px 20px; border-radius: 4px; cursor: pointer; }
          </style>
        </head>
        <body>
          <h1>无法连接到开发服务器</h1>
  ...
  // 如果有环境变量指定的URL，优先尝试该URL
  if (process.env.ELECTRON_START_URL) {
    console.log(`尝试加载环境变量指定的URL: ${startUrl}`);
    checkServer(startUrl, (available) => {
      clearTimeout(timeout); // 清除超时定时器

      if (available) {
        console.log("环境变量指定的服务器可用，加载URL:", startUrl);
        if (mainWindow) {
          mainWindow
            .loadURL(startUrl)
            .then(() => {
              console.log("环境变量指定的URL加载成功");
              // 确保窗口显示
              if (!mainWindow.isVisible()) {
                mainWindow.show();
                mainWindow.focus();
            ...
            .catch((err) => {
              console.error("加载环境变量指定的URL时出错:", err);
              // 如果加载失败，回退到尝试其他端口
              tryNextPort();
            });
        }
      } else {
        console.log(`环境变量指定的服务器不可用: ${startUrl}，尝试其他端口`);
        tryNextPort();
      }
    });
    return;
  ...
  // 如果没有环境变量指定的URL，尝试多个端口
  function tryNextPort() {
    if (currentPortIndex >= ports.length) {
      console.error("所有端口尝试失败，无法连接到开发服务器");
      showErrorPage();
      return;
    }

    const url = baseUrl + ports[currentPortIndex];
    console.log(`尝试加载URL: ${url}，重试次数: ${retryCount}/${maxRetries}`);
      ...
      if (available) {
        ...
        if (mainWindow) {
          mainWindow
            ...
            .catch((err) => {
              console.error("加载URL时出错:", err);
              currentPortIndex++;
              tryNextPort();
            });
        }
      } else {
        console.log(`服务器不可用: ${url}`);
        currentPortIndex++;
        tryNextPort();
      }
    });
