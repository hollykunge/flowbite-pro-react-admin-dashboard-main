 ...
 * @property {() => void} [onPlay] - 播放语音事件处理函数
 */
interface ChatVoiceMessageProps {
  avatarSrc: string;
  senderName: string;
...
/**
 * 聊天语音消息组件
 * 用于在聊天界面中显示语音类型的消息，包括发送者头像、名称、时间和语音波形图
 */
const ChatVoiceMessage: FC<ChatVoiceMessageProps> = ({
  avatarSrc,
  senderName,
  time,
  duration,
  ...
  return (
    <div className={`flex items-start gap-2.5 ${isOwn ? "justify-end" : ""}`}>
      ...
      <div className="flex flex-col gap-1">
        <div
          className={`flex items-center space-x-2 rtl:space-x-reverse ${isOwn ? "justify-end" : ""}`}
        >
        ...
        <VoiceMessage duration={duration} isOwn={isOwn} onPlay={onPlay} />
        {status && (
          <span
            className={`text-sm font-normal text-gray-500 dark:text-gray-400 ${isOwn ? "text-right" : ""}`}
          >
            {status}
          </span>
        )}
